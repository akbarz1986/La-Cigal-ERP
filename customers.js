const Customers = {
    ...
    render() {
        ...
        const todayMM = todayParts.find(p => p.type === 'month').value;
        const todayDD = todayParts.find(p => p.type === 'day').value;
        ...
        this.filteredData.forEach(cust => {
            ...
            if (cust.Birthday) {
                try {
                    ...
                    if (parts.length >= 2) {
                        const bdayMM = parts[1].padStart(2, '0');
                        const bdayDD = parts[2].substring(0, 2).padStart(2, '0');
                        const bdayMMDD = `${bdayMM}-${bdayDD}`;
            ...
            if (cust.Anniversary) {
                try {
                    const parts = cust.Anniversary.split('-');
                    if (parts.length >= 2) {
                        const annMM = parts[1].padStart(2, '0');
                        const annDD = parts[2].substring(0, 2).padStart(2, '0');
                        const annMMDD = `${annMM}-${annDD}`;

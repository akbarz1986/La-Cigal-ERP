...
function dispatchAction(action, payload, user) {
  switch (action) {
    ...
    case "updateAppointment":
      return updateAppointmentHandler(payload);
...
function getSheetRows(sheetName) {
  ...
  return values.map(row => {
    ...
    headers.forEach((header, index) => {
      // Convert Date objects to ISO strings or formatted dates in Asia/Karachi (Islamabad) timezone
      let val = row[index];
      if (val instanceof Date) {
        val = Utilities.formatDate(val, "Asia/Karachi", "yyyy-MM-dd");
      }
      obj[header] = val;
    });
...
function createBookingHandler(payload) {
  ...
  if (sheet) {
    ensureSheetHeaders(sheet, ["ApptID", "CustomerName", "Phone", "Date", "Time", "Services", "Staff", "Duration", "Status", "Notes", "AdvancePaid", "AdvanceMethod"]);
  }
  ...
  const newAppt = {
    ...
    AdvanceMethod: payload.advanceMethod || "None"
  };
...
/**
 * Updates an appointment booking.
 */
function updateAppointmentHandler(payload) {
  const apptId = payload.apptId;
  const updateData = payload.updateData || {};
  return updateSheetRow("Appointments", "ApptID", apptId, updateData);
}
...
function validateVoucherHandler(payload) {
  ...
  if (voucher.Expiry) {
    const todayStr = Utilities.formatDate(new Date(), "Asia/Karachi", "yyyy-MM-dd");
    if (voucher.Expiry < todayStr) {
...
function addExpenseHandler(payload) {
  ...
  const newExpense = {
    ExpenseID: nextId,
    Date: payload.date || Utilities.formatDate(new Date(), "Asia/Karachi", "yyyy-MM-dd"),
...
function updateSettingHandler(payload) {
  ...
  const updated = updateSheetRow("Settings", "Key", key, {
    Value: value
  });
  if (!updated) {
    const ss = getSpreadsheet();
